...
sed -i '587s/.*/\n/' /etc/squid/squid.conf
sed -i '587s/.*/\n/' /etc/squid/squid.conf
sed -i '587s/.*/thisiswhatwillgetreplaced/' /etc/squid/squid.conf
...
sed -i '588s/.*/auth_param basic realm proxy/' /etc/squid/squid.conf
sed -i '589s/.*/acl authenticated proxy_auth REQUIRED/' /etc/squid/squid.conf
